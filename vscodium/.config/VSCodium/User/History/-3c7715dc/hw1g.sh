...
selected_file=$(find "$search_path" -type f -not -path '*/\.git/*' -not -path '*/node_modules/*' -not -path '*/.cache/*' | fzf --preview "$preview_cmd" --preview-window=right:50%:wrap --header="Wybierz plik do otwarcia w nvim")
