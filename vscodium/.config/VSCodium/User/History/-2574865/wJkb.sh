...
arch_specific_configs() {
    ...
    if ! grep -q "ParallelDownloads" /etc/pacman.conf; then
        sudo sed -i '/\[options\]/a ParallelDownloads = 50' /etc/pacman.conf && success "Włączono równoległe pobieranie w pacman."
    fi
...
opensuse_specific_configs() {
    ...
    # Dodanie repozytoriów jeśli ich nie ma
    log "Konfiguracja repozytoriów..."
    if ! zypper lr | grep -q "Packman"; then
        sudo zypper ar -f http://ftp.gwdg.de/pub/linux/misc/packman/suse/openSUSE_Leap_$(lsb_release -rs)/ packman
        sudo zypper refresh
        # Wymiana pakietów na wersje z Packman
    ...
    # Optymalizacja btrfs (jeśli system plików to btrfs)
    if mount | grep -q "btrfs"; then
        log "Optymalizacja btrfs..."
        sudo mkdir -p /etc/systemd/system/fstrim.timer.d
        cat | sudo tee /etc/systemd/system/fstrim.timer.d/override.conf > /dev/null << EOF
[Timer]
OnCalendar=
OnCalendar=daily
EOF
