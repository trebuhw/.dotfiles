...
# Kompilacja i instalacja HYPRLAND
log "Kompilacja i instalacja HYPRLAND..."
cd ~/.config/suckless/HYPRLAND || { error "Nie można przejść do katalogu HYPRLAND"; exit 1; }
[ -f config.h ] && rm config.h
sudo make && sudo make clean install && rm -f config.h
check_success "Błąd podczas kompilacji HYPRLAND"
...
sudo cp ~/.config/suckless/usr/share/xsessions/HYPRLAND.desktop /usr/share/xsessions/
check_success "Nie udało się skopiować pliku .desktop"

log "Instalacja zakończona pomyślnie!"
log "Aby uruchomić HYPRLAND, wyloguj się i wybierz sesję HYPRLAND z menedżera logowania."
