...
install_gui_deps() {
    ...
    sudo xbps-install -Sy base-devel hyprland libX11-devel libXft-devel libXinerama-devel libdrm libgbm libinput \
        mesa-dri nvidia nvidia-libs wayland wayland-protocols wlroots xorg xorg-server xinit xwayland xf86-video-intel
...
void_specific_configs() {
    ...
    # W +Włączanie i uruchamianie usług (korzystamy z runit zamiast systemd)
