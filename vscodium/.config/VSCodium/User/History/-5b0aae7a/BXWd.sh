...
COMMON_PACKAGES=(
        ripgrep rofi rsync scrot stow sxhkd thunar thunar-archive-plugin thunar-volman time tree tumbler unrar unzip vim vlc wget xclip xdg-user-dirs xfce4-notifyd zathura
)

# Pakiety specyficzne dla Artix
PACMAN_PACKAGES=("${COMMON_PACKAGES[@]}" alacritty bash-completion blueman brightnessctl btop curl dunst fastfetch feh file-roller firefox font-manager fzf galculator gcc gnome-disk-utility gparted gsettings-desktop-schemas gzip htop i3lock libreoffice-fresh libreoffice-fresh-pl meld neovim network-manager-applet numlockx p7zip pavucontrol picom polkit-gnome network-manager-applet nsxiv plocate sddm tldr tlp qt5ct wezterm yazi)
