...
install_dwm_deps() {
    case "$DISTRO" in
        arch)
            ...
            # Instalacja paru dla Arch Linux
            install_paru() {
                if ! command -v paru &> /dev/null; then
                    log "Instalacja paru z AUR..."
                    tempdir=$(mktemp -d)
                    git clone https://aur.archlinux.org/paru.git "$tempdir/paru"
                    (
                        cd "$tempdir/paru" || exit 1
                        makepkg -si --noconfirm
                    )
                    rm -rf "$tempdir"
                    check_success "Nie udało się zainstalować paru"
                else
                    log "paru jest już zainstalowany"
                fi
            }          
