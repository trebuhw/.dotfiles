...
install_dwm_deps() {
    case "$DISTRO" in
        arch)
            ...
            log "Instalacja zależności HYPRLAND dla Arch Linux..."
            ...
            ;;
        ubuntu|debian)
            log "Instalacja zależności HYPRLAND dla $DISTRO..."
            sudo apt update
            sudo apt install -y 
            ;;
        fedora)
            log "Instalacja zależności HYPRLAND dla Fedora..."
            sudo dnf install -y 
            ;;
        opensuse)
            log "Instalacja zależności HYPRLAND dla openSUSE..."
            sudo sudo zypper --non-interactive install --no-recommends opi
            sudo zypper addrepo --refresh https://download.nvidia.com/opensuse/tumbleweed NVIDIA
            sudo zypper dup
            ;;
    esac
    check_success "Nie udało się zainstalować zależności HYPRLAND"
}
...
case "$DISTRO" in
    arch)
        ...
        acpi alacritty azote bash-completion bat blueman brightnessctl btop cliphist code cronie cups curl dunst eza fastfetch file-roller firefox fish font-manager foot fzf galculator gcc gcolor3 gimp gnome-disk-utility gparted grub-customizer gtk-engine-murrine gvfs gvfs-afc gvfs-dnssd gvfs-gphoto2 gvfs-mtp gvfs-nfs gvfs-smb htop hyprland hyprpicker i3lock kitty libreoffice-fresh libreoffice-fresh-pl lm_sensors mako mc meld mlocate mousepad nano neovim network-manager-applet nsxiv numlockx nwg-look os-prober pamixer parcellite pavucontrol pdfarranger picom polkit-gnome qt5ct ranger rclone ripgrep rofi rofi-emoji rsync sane sddm scrot simple-scan slurp speedtest-cli starship stow swappy swaybg system-config-printer tealdeer thunar thunar-archive-plugin thunar-volman time timeshift tlp trash-cli tree tumbler udiskie unrar unzip vlc vim waybar wezterm wget wl-clipboard wofi xclip xdg-user-dirs xorg-xhost xreader yazi yt-dlp zathura zathura-pdf-mupdf zenity zoxide
        )
        YAY_PACKAGES=(
        brother-dcp1610w brscan4 google-chrome grimblast-git mkinitcpio-firmware moc-pulse simple-sddm-theme-git sublime-text-4 waypaper wlogout
            )
        ;;
    ubuntu)
        PACMAN_PACKAGES=("${COMMON_PACKAGES[@]}" 
    ...
    opensuse)
        PACMAN_PACKAGES=("${COMMON_PACKAGES[@]}"
        acpi alacritty azote bash-completion bat blueman brightnessctl btop cliphist code cpuid cups curl dconf-editorduf dunst eza fastfetch feh file-roller firefox fish font-manager foot fzf galculator gcc gcolor3 gimp git gnome-disk-utility gparted gsettings-desktop-schemas grub-customizer gtk-engine-murrine gvfs gvfs-afc gvfs-dnssd gvfs-gphoto2 gvfs-mtp gvfs-nfs gvfs-smb gzip htop hyprland hyprpicker i3lock kitty libreoffice libreoffice-l10n-pl mako mc meld mlocate mousepad nano neovim NetworkManager-applet network-manager-applet nwg-look opi os-prober p7zip pamixer pavucontrol pdfarranger picom polkit polkit-gnome qt5ct ranger ripgrep rofi rofi-emoji rsync sane sddm scrot sensors simple-scan slurp speedtest-cli starship stow swappy swaybg sxiv system-config-printer tealdeer thunar thunar-archive-plugin thunar-volman time tlp trash-cli tree tumbler ueberzugpp udiskie unrar unzip vim vlc waybar wezterm wget wl-clipboard wofi xclip xdg-user-dirs xfce4-notifyd xreader yazi yt-dlp zathura zathura-pdf-mupdf zathura-plugin-pdf-poppler zenity zoxide kernel-firmware-nvidia libnvidia-egl-wayland1 nvidia-compute-G06 nvidia-compute-G06-32bit nvidia-driver-G06-kmp-default nvidia-gl-G06 nvidia-gl-G06-32bit nvidia-video-G06 nvidia-video-G06-32bi
            )   
        ;;
esac
...
stow Xresources/ alacritty/ background/ bash/ bin/ btop/ dunst/ fish/ foot/ fonts/ gtk-2.0/ gtk-3.0/ gtk-4.0/ gtkrc-2.0/ hypr/ icons/ kitty/ mako/ mc/ mousepad/ nsxiv/ nvim/ qt5ct/ profile/ ranger/ rofi/ sublime-text/ swappy/ swaylock/ tldr/ themes/ thunar/ vim/ vscode/ waybar/ wezterm/ wlogout/ wofi/ wofifull/ xfce4/ yazi/ zathura/
