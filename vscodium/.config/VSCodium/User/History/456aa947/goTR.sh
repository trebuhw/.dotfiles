...
case "$DISTRO" in
    ...
    opensuse)
        PACMAN_PACKAGES=("${COMMON_PACKAGES[@]}"
        opi openSUSE-repos-Tumbleweed-NVIDIA openSUSE-repos-MicroOS-NVIDIA acpi alacritty azote bash-completion bat blueman brightnessctl btop cliphist code cpuid cups curl dconf-editorduf dunst eza fastfetch feh file-roller firefox fish font-manager foot fzf galculator gcc gcolor3 gimp git gnome-disk-utility gparted gsettings-desktop-schemas grub-customizer gtk-engine-murrine gvfs gvfs-afc gvfs-dnssd gvfs-gphoto2 gvfs-mtp gvfs-nfs gvfs-smb gzip harfbuzz-devel htop hyprland hyprpicker i3lock kitty libreoffice libreoffice-fresh libreoffice-fresh-pl libreoffice-l10n-pl libX11-devel libXft-devel libXinerama-devel libxcb-res0 lightdm lightdm-gtk-greeter-settings lm_sensors lsd mako mc meld mlocate mousepad nano neovim NetworkManager-applet ncurses-devel network-manager-applet nsxiv numlockx nwg-look os-prober p7zip pamixer papirus-icon-theme parcellite pavucontrol pdfarranger picom polkit polkit-gnome qt5ct ranger rclone ripgrep rofi rofi-emoji rsync sane sddm scrot sensors simple-scan slurp speedtest-cli starship stow swappy swaybg sxhkd sxiv system-config-printer tealdeer tealdeer-fish-completion thunar thunar-archive-plugin thunar-volman time timeshift tlp trash-cli tree tumbler ueberzugpp udiskie unrar unzip vim vlc waybar wezterm wget wl-clipboard wofi xclip xdg-user-dirs xf86-video-intel xfce4-notifyd xreader xwininfo yazi yt-dlp zathura zathura-pdf-mupdf zathura-plugin-pdf-poppler zenity zoxide kernel-firmware-nvidia libnvidia-egl-wayland1 nvidia-compute-G06 nvidia-compute-G06-32bit nvidia-driver-G06-kmp-default nvidia-gl-G06 nvidia-gl-G06-32bit nvidia-video-G06 nvidia-video-G06-32bit
...
install_repo_packages() {
    ...
    case "$DISTRO" in
        ...
        opensuse)
            ...
            sudo opi azote cliphist code eza fastfetch foot hyprland hyprpicker libreoffice-fresh libreoffice-fresh-pl mako nsxiv nwg-look pamixer parcellite rofi-emoji slurp swappy tealdeer waybar wezterm wofi yazi 
            ;;
    esac
}
