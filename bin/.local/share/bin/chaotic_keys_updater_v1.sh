    ...
    for mirror in "${mirrors[@]}"; do
        ...
        if mirror_content=$(curl -s --connect-timeout 10 "$mirror/" 2>/dev/null); then
            ...
            if [[ -n "$keyring_file" && -n "$mirrorlist_file" ]]; then
                log "Znalezione pliki: $keyring_file, $mirrorlist_file"
        ...
        warning "Nie udało się pobrać z $mirror"
    done
...
show_keys_info() {
    ...
    if pacman -Q chaotic-keyring &> /dev/null; then
        echo -e "${GREEN}[OK]${NC} chaotic-keyring: $(pacman -Q chaotic-keyring | awk '{print $2}')"
    else
        echo -e "${RED}[BRAK]${NC} chaotic-keyring: nie zainstalowany"
    fi
    
    if pacman -Q chaotic-mirrorlist &> /dev/null; then
        echo -e "${GREEN}[OK]${NC} chaotic-mirrorlist: $(pacman -Q chaotic-mirrorlist | awk '{print $2}')"
    else
        echo -e "${RED}[BRAK]${NC} chaotic-mirrorlist: nie zainstalowany"
    fi
...
main() {
    ...
    if update_via_pacman; then
        verify_update
        show_keys_info
        success "Aktualizacja kluczy zakończona pomyślnie!"
        return 0
    fi
...
case "${1:-}" in
    ...
    --help|-h)
        ...
        echo ""
        echo "Użycie:"
        echo "  $0              - Aktualizuj klucze Chaotic-AUR"
        ...
        echo "  - Automatyczna aktualizacja kluczy Chaotic-AUR"
        echo "  - Weryfikacja z różnych mirrorów"
        echo "  - Obsługa problemów z kluczami GPG"
        ;;
    "")
        main
        ;;
